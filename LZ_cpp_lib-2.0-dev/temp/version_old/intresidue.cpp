...
//public:
	...
	():
	m_ptr_ring(type_this::val_ptr_ring_universal),m_value(type_val(0)){
	}
	...
	Element<IntType,RingType,TraitsCheck>::Element
	(AuxIntType const & num_value):
	m_ptr_ring(type_this::val_ptr_ring_universal),m_value(type_val(num_value)){
	}
	
	template<typename IntType,typename RingType,typename TraitsCheck>
	template<typename AuxIntType,typename>
	Element<IntType,RingType,TraitsCheck>::Element
	(AuxIntType const & num_value,type_ring const & ring):
	m_ptr_ring(),
	m_value(ring.Is_finite() ? type_val(mod(num_value,ring.modulo())) : type_val(num_value)){
		if(ring.Is_specific()){
			m_ptr_ring=make_shared<type_ring const>(ring);
		} else if(ring.Is_universal()){
			m_ptr_ring=type_this::val_ptr_ring_universal;
		} else {
			m_ptr_ring=type_this::val_ptr_ring_invalid;
		}
	}
	...
	template<typename AuxIntType,typename>
	Element<IntType,RingType,TraitsCheck>::Element
	(AuxIntType const & num_value,type_ptr_ring const & ptr_ring):
	m_ptr_ring(),
	m_value((ptr_ring && ptr_ring->Is_finite()) ?
	type_val(mod(num_value,ptr_ring->modulo())) : type_val(num_value)){
		if(!ptr_ring || ptr_ring->Is_invalid()){
			m_ptr_ring=type_this::val_ptr_ring_invalid;
		} else if(ptr_ring->Is_specific()){
			m_ptr_ring=ptr_ring;
		} else {
			m_ptr_ring=type_this::val_ptr_ring_universal;
		}
	...
	m_ptr_ring(element.m_ptr_ring),
	m_value(element.Is_invalid() ? element.m_value :
	(element.m_ptr_ring->Is_finite() ?
	type_val(mod(num_value,element.m_ptr_ring->modulo())) : type_val(num_value))){
	}
	
	template<typename IntType,typename RingType,typename TraitsCheck>
	Element<IntType,RingType,TraitsCheck>::Element
	(type_this const & element):
	m_ptr_ring(element.m_ptr_ring),m_value(element.m_value){
	}
	...
	(AuxIntType const & x){
		...
		m_value= m_ptr_ring->Is_finite() ?
			(m_ptr_ring->modulo()==type_mod(1) ?
			type_val(0) : type_val(mod(x,m_ptr_ring->modulo()))) :
			type_val(x);
		return *this;
	}
	...
	(type_this const & x){
		...
		if(*(this->m_ptr_ring)!=*(x.m_ptr_ring)) return *this=type_this::invalid(type_val(2));
		if(this->Is_universal() && x.Is_specific()){
			this->m_ptr_ring=x.m_ptr_ring;
		}
		if(this->m_ptr_ring->modulo()==type_mod(1)){
			this->m_value=type_val(0);
			return *this;
		}
		this->m_value+=x.m_value;
		if(this->m_ptr_ring->Is_finite()){
			this->m_value=mod(this->m_value,this->m_ptr_ring->modulo());
		}
		return *this;
	}
	...
	(AuxIntType const & num_times){
		...
		if(this->m_ptr_ring->Is_finite()){
			this->m_value=Liuze::math::mod(this->m_value,this->m_ptr_ring->modulo());
		}
		return *this;
	}
	...
	(){
		if(this->Is_invalid()) return *this;
		++this->m_value;
		if(this->space().Is_finite() && this->m_value==type_val(this->space().modulo())){
			this->m_value=type_val(0);
		}
		return *this;
	}
	...
	(){
		if(this->Is_invalid()) return *this;
		if(this->space().Is_finite() && this->m_value==type_val(0)){
			this->m_value=type_val(this->space().modulo());
		}
		--this->m_value;
		return *this;
	}
	...
	() const {
		...
		type_this res=*this;
		res.m_value= this->m_ptr_ring->Is_finite() ?
			type_val(res.space().modulo())-res.m_value : -res.m_value;
		return *this;
	}
	...
	(type_this const & x) const {
		return (this->Is_invalid() || x.Is_invalid()) ? (this->Is_invalid() && x.Is_invalid()) :
			(this->m_value==x.m_value && *(this->m_ptr_ring)==*(x.m_ptr_ring));
	}
	...
	() const {
		return this->m_ptr_ring->Is_valid();
	}
	...
	() const {
		return this->m_ptr_ring->Is_invalid();
	}
	...
	() const {
		return this->m_ptr_ring->Is_universal();
	}
	...
	() const {
		return this->m_ptr_ring->Is_specific();
	}
	...
	() const {
		...
		if(this->Is_universal() || this->m_ptr_ring->modulo()==type_mod(0)){
			return abs(this->m_value)==type_val(1);
		}
		type_val c0=0;
		if(this->m_ptr_ring->modulo()==type_mod(1) || this->m_value==c0) return false;
		type_val n0=this->m_ptr_ring->modulo(),n1=this->m_value;
		type_val q=n0/n1;
		type_val r=n0-q*n1;
		while(r!=c0){
			n0=n1;
	...
	template<typename IntType,typename RingType,typename TraitsCheck>
	inline
	typename Element<IntType,RingType,TraitsCheck>::type_ring const &
	Element<IntType,RingType,TraitsCheck>::space
	() const {
		return *(this->m_ptr_ring);
	}
	
	template<typename IntType,typename RingType,typename TraitsCheck>
	inline
	typename Element<IntType,RingType,TraitsCheck>::type_ptr_ring const &
	Element<IntType,RingType,TraitsCheck>::ptr_space
	() const {
		return this->m_ptr_ring;
	}
	...
	template<typename IntType,typename RingType,typename TraitsCheck>
	typename Element<IntType,RingType,TraitsCheck>::type_this
	Element<IntType,RingType,TraitsCheck>::zero
	() const {
	...
	template<typename IntType,typename RingType,typename TraitsCheck>
	typename Element<IntType,RingType,TraitsCheck>::type_this
	Element<IntType,RingType,TraitsCheck>::one
	() const {
	...
	() const {
		...
		type_val n0=this->m_ptr_ring->modulo(),n1=this->m_value,coef10=c0,coef11=c1;
		type_val q=n0/n1;
		type_val r=n0-q*n1;
		while(r!=c0){
			n0=n1;
	...
	(AuxIntType const & exp) const {
		if(this->Is_invalid() || (this->Is_specific() && this->m_ptr_ring->modulo()==type_mod(1))){
			return *this;
		}
		if(exp<AuxIntType(0)){
	...
	(){
		return type_this(type_val(0),type_this::val_ptr_ring_invalid);
	}
	...
	Element<IntType,RingType,TraitsCheck>::invalid
	(AuxIntType const & num_value){
		return type_this(type_val(num_value),type_this::val_ptr_ring_invalid);
	}
	...
	(AuxIntType const & num_value){
		return type_this(type_val(num_value),type_this::val_ptr_ring_universal);
	}
...
//protected:
	...
	template<typename IntType,typename RingType,typename TraitsCheck>
	typename Element<IntType,RingType,TraitsCheck>::type_ptr_ring
	Element<IntType,RingType,TraitsCheck>::val_ptr_ring_invalid
	=std::make_shared<type_ring const>(type_ring::invalid());
	
	template<typename IntType,typename RingType,typename TraitsCheck>
	typename Element<IntType,RingType,TraitsCheck>::type_ptr_ring
	Element<IntType,RingType,TraitsCheck>::val_ptr_ring_universal
	=std::make_shared<type_ring const>(type_ring::universal());
