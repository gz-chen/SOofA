...
//public:
	...
	(type_val const & num_value) const {
		...
		typename type_int_coef::type_ptr_ring ptr_ring_coef=
			std::make_shared<typename type_int_coef::type_ring const>(this->characteristic());
		Type_ArrayTemp<type_int_coef> arr_coef(ord);
		type_val i_ord,j_ord=ord;
		for(i_ord=0;i_ord<ord;++i_ord){
			--j_ord;
			arr_coef[j_ord]=type_int_coef(arr_coef_origin[i_ord],ptr_ring_coef);
		}
	...
	(AuxIntType const & num_value){
		return type_element::universal(type_val(num_value));
	}
	...
	(type_val const & num_prime,type_val const & num_degree_upper){
		...
		if(num_degree_upper==type_val(1)){
			...
			typename type_int_coef::type_ptr_ring ptr_ring_coef=
				std::make_shared<typename type_int_coef::type_ring const>(num_prime);
			Type_ArrayTemp<type_int_coef> arr_coef(2);
			arr_coef[1]=type_int_coef(type_val(1),ptr_ring_coef);
			for(type_val i_poly=0;i_poly<num_prime;++i_poly){
				arr_coef[0]=type_int_coef(type_val(i_poly),ptr_ring_coef);
		...
		for(i_deg=1;i_deg<=num_degree_upper;++i_deg){
			...
			arr_coef[i_deg]=type_int_coef(type_val(1),ptr_ring_coef); //monic;
			...
			while(iter_pow.Is_stat_deref()){
				for(i_coef=1;i_coef<i_deg;++i_coef){
					arr_coef[i_deg-i_coef]=type_int_coef((*iter_pow)[i_coef-type_val(1)],ptr_ring_coef);
				}
				for(i_coef=i_poly;i_coef<num_prime;++i_coef){
					arr_coef[0]=type_int_coef(i_coef,ptr_ring_coef);
...
//public:
	...
	(FieldType && field):
	m_tab_add(0),m_tab_neg(0),m_tab_pow(0),m_tab_log(0),
	type_base(std::forward<FieldType>(field)){
		this->Cal_table();
	}
...
//protected:
	...
	(){
		...
		typename type_base_element::type_ptr_space ptr_base_space=
			std::make_shared<type_base const>(*this);
		Type_ArrayTemp<type_base_element> list_base_element(this->size());
		for(i_ele=0;i_ele<this->size();++i_ele){
			list_base_element[i_ele]=type_base_element(i_ele,ptr_base_space);
		}
...
//public:
	...
	():
	m_ptr_field(type_this::val_ptr_field_universal),m_val_num(0),m_val_poly(){
	}
	...
	Element<IntType,FieldType,TraitsCheck>::Element
	(AuxIntType const & num_value):
	m_ptr_field(type_this::val_ptr_field_universal),m_val_num(num_value),m_val_poly(){
	}
	
	template<typename IntType,typename FieldType,typename TraitsCheck>
	template<typename AuxIntType,typename>
	Element<IntType,FieldType,TraitsCheck>::Element
	(AuxIntType const & num_value,type_field const & field):
	m_ptr_field(),m_val_num(num_value),m_val_poly(){
		if(field.Is_specific()){
			m_ptr_field=std::make_shared<type_field const>(field);
			this->Set_init_val();
		} else if(field.Is_universal()){
			m_ptr_field=type_this::val_ptr_field_universal;
		} else {
			m_ptr_field=type_this::val_ptr_field_invalid;
		}
	}
	...
	template<typename AuxIntType,typename>
	Element<IntType,FieldType,TraitsCheck>::Element
	(AuxIntType const & num_value,type_ptr_field const & ptr_field):
	m_ptr_field(),m_val_num(num_value),m_val_poly(){
		if(!ptr_field || ptr_field->Is_universal()){
			m_ptr_field=type_this::val_ptr_field_universal;
		} else if(ptr_field->Is_specific()){
			m_ptr_field=ptr_field;
			this->Set_init_val();
		} else {
			m_ptr_field=type_this::val_ptr_field_invalid;
		}
	...
	m_ptr_field(element.m_ptr_field),m_val_num(num_value),m_val_poly(){
		this->Set_init_val();
	}
	...
	m_ptr_field(element.m_ptr_field),
	m_val_num(element.m_val_num),m_val_poly(element.m_val_poly){
	}
	...
	Element<IntType,FieldType,TraitsCheck>::operator =
	(AuxIntType const & num_value){
		if(m_val_num==type_val(num_value)) return *this;
	...
	(type_this const & x){
		...
		if(!(x.Is_specific())){
			type_this x_new(x.m_val_num,this->m_ptr_field);
			this->m_val_poly+=x_new.m_val_poly;
		} else {
			if(!(this->Is_specific())){
				this->m_ptr_field=x.m_ptr_field;
				this->Set_init_val();
			} else if(*(this->m_ptr_field)!=*(x.m_ptr_field)){
				this->Set_invalid();
				return *this;
			}
			this->m_val_poly+=x.m_val_poly;
		}
		this->m_val_num=this->m_ptr_field->poly_to_num(this->m_val_poly);
		return *this;
	}
	...
	(type_this const & x){
		...
		this->m_val_poly%=this->m_ptr_field->irrPoly_cref();
		this->m_val_num=this->m_ptr_field->poly_to_num(this->m_val_poly);
		return *this;
	}
	...
	() const {
		if(this->Is_invalid()) return *this;
		type_this res;
		res.m_ptr_field=this->m_ptr_field;
		if(this->Is_specific()){
			res.m_val_poly=-(this->m_val_poly);
			res.m_val_num=res.m_ptr_field->poly_to_num(res.m_val_poly);
		} else {
	...
	(type_this const & element) const {
		return (this->Is_invalid() || element.Is_invalid()) ?
			...
			(*(this->m_ptr_field)==*(element.m_ptr_field))) : false);
	}
	...
	() const {
		return m_ptr_field->Is_universal();
	}
	...
	() const {
		return m_ptr_field->Is_specific();
	}
	...
	() const {
		return (this->Is_valid() &&
			...
			(this->Is_specific() && this->m_ptr_field->size()==type_val(1))));
	}
	...
	template<typename IntType,typename FieldType,typename TraitsCheck>
	inline
	typename Element<IntType,FieldType,TraitsCheck>::type_field const &
	...
	template<typename IntType,typename FieldType,typename TraitsCheck>
	inline
	typename Element<IntType,FieldType,TraitsCheck>::type_ptr_field const &
	Element<IntType,FieldType,TraitsCheck>::ptr_space
	() const {
		return this->m_ptr_field;
	}
	...
	() const {
		return type_this(Liuze::math::zero<type_val>(),this->ptr_space());
	}
	...
	() const {
		return type_this(Liuze::math::one<type_val>(),this->ptr_space());
	}
	...
	Element<IntType,FieldType,TraitsCheck>::element
	(AuxIntType const & num_value) const {
		return type_this(type_val(num_value),this->ptr_space());
	}
	...
	() const {
		...
		if(this->m_ptr_field->size()==type_val(1)) return *this;
		if(this->Is_zero()) return type_this::invalid();
		type_poly p0=this->m_ptr_field->irrPoly_cref();
		type_poly p1=this->m_val_poly;
		type_poly c1=this->m_val_poly.constant
			(type_int_coef(type_val(1),
			typename type_int_coef::type_ptr_ring(
			std::make_shared<typename type_int_coef::type_ring const>(
			this->m_ptr_field->characteristic()))));
	...
	(){
		this->m_ptr_field=type_this::val_ptr_field_invalid;
		this->Set_init_val();
		return *this;
	}
	...
	(){
		return type_this(type_val(0),type_this::val_ptr_field_invalid);
	}
	...
	(AuxIntType const & num_value){
		return type_this(type_val(num_value),type_this::val_ptr_field_universal);
	}
...
//protected:
	...
	template<typename IntType,typename FieldType,typename TraitsCheck>
	typename Element<IntType,FieldType,TraitsCheck>::type_ptr_field
	Element<IntType,FieldType,TraitsCheck>::val_ptr_field_invalid
	=std::make_shared<type_field const>(type_field::invalid());
	
	template<typename IntType,typename FieldType,typename TraitsCheck>
	typename Element<IntType,FieldType,TraitsCheck>::type_ptr_field
	Element<IntType,FieldType,TraitsCheck>::val_ptr_field_universal
	=std::make_shared<type_field const>(type_field::universal());
...
//private:
	...
	(){
		...
		if(m_val_num>=type_val(0)){
			m_val_num=Liuze::math::mod(m_val_num,m_ptr_field->size());
			m_val_poly=m_ptr_field->num_to_poly(m_val_num);
		} else {
			m_val_num=Liuze::math::mod(-m_val_num,m_ptr_field->size());
			m_val_poly=-(m_ptr_field->num_to_poly(m_val_num));
			m_val_num=m_ptr_field->poly_to_num(m_val_poly);
		}
		return;
	}
...
//public:
	...
	template<typename IntType,typename CoefRingType>
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Element
	():
	m_ptr_field(type_this::val_ptr_field_universal),m_val_num(0){
	}
	...
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Element
	(AuxIntType const & num_value):
	m_ptr_field(type_this::val_ptr_field_universal),m_val_num(num_value){
	}
	...
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Element
	(AuxIntType const & num_value,type_field const & field):
	m_ptr_field(),m_val_num(num_value){
		if(field.Is_specific()){
			m_ptr_field=std::make_shared<type_field const>(field);
			this->Set_init_val();
		} else if(field.Is_universal()){
	...
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Element
	(AuxIntType const & num_value,type_this const & element):
	m_ptr_field(element.m_ptr_field),m_val_num(num_value){
		this->Set_init_val();
	}
	...
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Element
	(type_this const & element):
	m_ptr_field(element.m_ptr_field),m_val_num(element.m_val_num){
	}
	...
	template<typename IntType,typename CoefRingType>
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this &
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::operator =
	(type_this const & element){
		if(this==std::addressof(element)) return *this;
		m_ptr_field=element.m_ptr_field;
		m_val_num=element.m_val_num;
	...
	(type_this const & x){
		...
		type_val val_x;
		if(!(x.Is_specific())){
			val_x=type_this(x.m_val_num,this->m_ptr_field).m_val_num;
		} else {
			if(!(this->Is_specific())){
				this->m_ptr_field=x.m_ptr_field;
				this->Set_init_val();
		...
		this->m_val_num=this->m_ptr_field->table_add(((val_x+type_val(1))*val_x)/type_val(2)+val_y);
		return *this;
	}
	...
	(type_this const & x){
		...
		if(!(x.Is_specific())){
			if(this->Is_zero()) return *this;
			pow_x=type_this(x.m_val_num,this->m_ptr_field).m_val_num;
			if(pow_x==type_val(0)){
				this->m_val_num=pow_x;
				return *this;
			}
			pow_x=this->m_ptr_field->table_log(pow_x-type_val(1));
		} else {
			if(!(this->Is_specific())){
				this->m_ptr_field=x.m_ptr_field;
				this->Set_init_val();
			...
			if(this->Is_zero()) return *this;
			if(x.Is_zero()) return *this=x;
			pow_x=this->m_ptr_field->table_log(x.m_val_num-type_val(1));
		}
		type_val pow_this=this->m_ptr_field->table_log(this->m_val_num-type_val(1));
		this->m_val_num=this->m_ptr_field->table_pow
			(Liuze::math::mod(pow_this+pow_x,this->m_ptr_field->size()-type_val(1)));
		return *this;
	}
	...
	() const {
		...
		res.m_ptr_field=this->m_ptr_field;
		if(this->Is_specific()){
			res.m_val_num=this->m_ptr_field->table_neg(this->m_val_num);
		} else {
			res.m_val_num=-(this->m_val_num);
		}
		return res;
	...
	template<typename IntType,typename CoefRingType>
	inline
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::operator Type_Bool
	() const {
		return (this->Is_valid() && this->m_val_num!=type_val(0));
	}
	...
	template<typename AuxType,typename>
	inline
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::operator AuxType
	() const {
		return AuxType(this->value_number());
	}
	...
	inline
	Type_Bool
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Is_invalid
	() const {
		return m_ptr_field->Is_invalid();
	}
	...
	inline
	Type_Bool
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Is_valid
	() const {
		return m_ptr_field->Is_valid();
	}
	...
	inline
	Type_Bool
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Is_universal
	() const {
		return m_ptr_field->Is_universal();
	}
	...
	inline
	Type_Bool
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Is_specific
	() const {
		return m_ptr_field->Is_specific();
	}
	...
	inline
	Type_Bool
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Is_zero
	() const {
		return (this->Is_valid() && this->m_val_num==type_val(0));
	}
	...
	inline
	Type_Bool
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Is_one
	() const {
		return (this->Is_valid() &&
			(this->m_val_num==type_val(1) ||
			(this->Is_specific() && this->m_ptr_field->size()==type_val(1))));
	...
	template<typename IntType,typename CoefRingType>
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_val
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::value_number
	() const {
		return this->Is_valid() ? this->m_val_num : type_val(0);
	}
	...
	template<typename IntType,typename CoefRingType>
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_poly
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::value_polynomial
	() const {
		return this->Is_specific() ?
			this->m_ptr_field->num_to_poly(this->m_val_num) : type_poly();
	}
	
	template<typename IntType,typename CoefRingType>
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_field const &
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::space
	() const {
		return *(this->m_ptr_field);
	}
	...
	template<typename IntType,typename CoefRingType>
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_ptr_field const &
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::ptr_space
	() const {
		return this->m_ptr_field;
	}
	...
	template<typename IntType,typename CoefRingType>
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::zero
	() const {
		return type_this(Liuze::math::zero<type_val>(),this->ptr_space());
	}
	...
	template<typename IntType,typename CoefRingType>
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::one
	() const {
		return type_this(Liuze::math::one<type_val>(),this->ptr_space());
	}
	...
	template<typename IntType,typename CoefRingType>
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::inv
	() const {
		if(this->Is_invalid()) return *this;
		if(this->Is_universal()){
			if(this->Is_one() || (-(*this)).Is_one()) return *this;
		...
		if(this->m_ptr_field->size()==type_val(1)) return *this;
		if(this->Is_zero()) return type_this::invalid();
		type_val log_this=this->m_ptr_field->table_log(this->m_val_num-type_val(1));
		if(log_this>type_val(0)){
			log_this=this->m_ptr_field->size()-type_val(1)-log_this;
		}
		return type_this(this->m_ptr_field->table_pow(log_this),this->m_ptr_field);
	}
	
	template<typename IntType,typename CoefRingType>
	template<typename AuxIntType,typename>
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::pow
	(AuxIntType const & num_exp) const {
		if(this->Is_invalid()) return *this;
		if(num_exp==AuxIntType(0)) return Liuze::math::one(*this);
		if(num_exp==AuxIntType(1)) return *this;
		...
		//now `*this' is specific and is not 0, -1 or 1, and `num_exp' is not 0 or 1.
		return type_this(
			this->ptr_space()->table_pow(Liuze::math::mod(
			(this->ptr_space()->table_log(this->m_val_num-type_val(1)))*type_val(num_exp),
			this->ptr_space()->size()-type_val(1))),
			this->ptr_space());
	}
	...
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this &
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Set_invalid
	(){
		this->m_ptr_field=type_this::val_ptr_field_invalid;
		this->Set_init_val();
		return *this;
	...
	template<typename InputIterType,typename OutputIterType,typename>
	void
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::convert_to_value_number
	(InputIterType const & first,InputIterType const & last,OutputIterType result) const {
		typedef typename OutputIterType::value_type type_out;
		for(InputIterType iter=first;iter!=last;++iter){
			*result=type_out(iter->value_number());
	...
	template<typename InputIterType,typename OutputIterType,typename>
	void
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::convert_to_value_polynomial
	(InputIterType const & first,InputIterType const & last,OutputIterType result) const {
		typedef typename OutputIterType::value_type type_out;
		for(InputIterType iter=first;iter!=last;++iter){
			*result=type_out(iter->value_polynomial());
	...
	template<typename InputIterType,typename OutputIterType,typename>
	void
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::convert_from_value_number
	(InputIterType const & first,InputIterType const & last,OutputIterType result) const {
		for(InputIterType iter=first;iter!=last;++iter){
			*result=type_this(type_val(*iter),*this);
			++result;
	...
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::invalid
	(){
		return type_this(type_val(0),type_this::val_ptr_field_invalid);
	}
	...
	inline
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_this
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::universal
	(AuxIntType const & num_value){
		return type_this(type_val(num_value),type_this::val_ptr_field_universal);
	}
...
//protected:
	...
	template<typename IntType,typename CoefRingType>
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_ptr_field
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::val_ptr_field_invalid
	=std::make_shared<type_field const>(type_field::invalid());
	
	template<typename IntType,typename CoefRingType>
	typename Element<IntType,Field_tabulated<IntType,CoefRingType> >::type_ptr_field
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::val_ptr_field_universal
	=std::make_shared<type_field const>(type_field::universal());
...
//private:
	...
	inline
	void
	Element<IntType,Field_tabulated<IntType,CoefRingType> >::Set_init_val
	(){
		if(this->Is_invalid()){
			m_val_num=0;
			return;
		...
		if(m_val_num>=type_val(0)){
			...
			m_val_num=m_ptr_field->table_neg(Liuze::math::mod(-m_val_num,m_ptr_field->size()));
		}
		return;
	}
...
namespace Eigen{
	...
	public GenericNumTraits<Liuze::math::GaloisField::Element<IntType,FieldType> >{
		public:
			typedef Liuze::math::GaloisField::Element<IntType,FieldType> type_num;
